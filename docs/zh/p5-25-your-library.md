...
**自包含是分享的超能力。** 因为 Workflow 脚本**没有** `import`、**不碰**文件系统、**不依赖** Node API（B 节硬约束：「无文件系统/Node API」「标准 JS 内置可用」），一个 `.js` 文件就是完整的、可移植的、可审计的。收到别人的工作流脚本，你能一眼读完它要派几个 agent、用什么 schema、有没有无界循环——**它的全部行为都在那一个文件里**。这是把它收进自己库之前，做第 24 章「解构」的理想素材。
