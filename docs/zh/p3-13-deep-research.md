...
这是深度研究配方的**灵魂**，值得单独拆开讲。LLM 检索的头号风险不是「查不到」，而是「查到一个似是而非的二手转述，并自信地复述」。配方用三道防线把这种 claim 拦下来：
...
**实战食谱篇（10–16）至此全部完结**，每一篇都锚定了真实运行。第四部转向让这些配方更可信的进阶模式，第一站正是把本章与第 15 章的「对抗证伪」抽象成通用模式。
