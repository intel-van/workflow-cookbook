...
>
> The warp tensioned, what truly makes a bolt of cloth "grow a pattern" is that weft shuttling back and forth. In Workflow, this weft is `agent()` — it dispatches a subagent to do one concrete job, waits for it to return, and hands you the product.
>
> In the last chapter we took the structural skeleton of `meta` and `phase()` apart down to the bottom. This chapter turns its full gaze on `agent(prompt, opts)`: what exactly its return value is (text? object? or `null`?), what problem each of its options (`label`, `schema`, `phase`, `model`, `isolation`, `agentType`) solves, and why the subagent it dispatches can **protect your main loop's context.**
>
> This is the most-used function in the whole book. Master it, and all the later practical recipes are just different orchestrations of it.
...
- Second parameter `opts`: an optional options object, tuning this agent's behavior.
...
It dispatches a subagent, runs to completion, and returns **a piece of text.** This is `agent()`'s plainest form. Next let's first explain the most crucial thing — "what it returns" — because the return semantics determine how you write the code that follows.

---

## 6.2 Return Semantics: Text, Object, or null?

`agent()`'s return value has **three** possibilities, depending on how you call it and how the user responds. Confuse these three, and the subsequent `.filter()`, destructuring, and `JSON.parse` all go wrong. Per `_grounding.md` section B, the rules are:
...
This suits the "I just want a natural-language result" scenario: summary, explanation, drafting a piece of text. What you get is the last paragraph the subagent wrote.
...
Note `sum` is the number `4`, **not the string `"4"`** — because the schema declared `type: 'number'` and the validation layer ensured the type. You can directly do `r.sum + 1` arithmetic, with no parsing and no error handling. This mechanism (and how it forces retries at the tool-call layer) is the subject of Chapter 07; here just remember: **has schema → you get a clean object you can directly destructure and compute on.**

### 6.2.3 User skips → returns null

The third case is most easily overlooked: **the user skips this agent midway** (e.g., choosing to skip a step in the interaction), and `agent()` returns `null`.

Per `_grounding.md`: "user skips the agent midway → returns `null` (filter with `.filter(Boolean)`)."

This is why all of this book's real `parallel()` / `pipeline()` examples are nearly always followed by a `.filter(Boolean)` before using the results:
...
`.filter(Boolean)` is an idiom: `Boolean` as the filter function strips out all "falsy values" in the array (`null`, `undefined`, `0`, `''`, `false`). Here its purpose is to **clean out the skipped `null` items**, leaving only the ones with real results.

<div class="callout warn">

**Use it directly without `.filter(Boolean)`, and `null` will bite you.** If you `results.map(r => r.findings)` and some `r` is `null`, it throws `Cannot read properties of null`. Form the habit: **for any `parallel` / `pipeline` result, `.filter(Boolean)` before consuming.** A single `await agent(...)` is the same — if it might be skipped, check `if (r) { ... }` before use.
...
`label` is the simplest option: it overrides this agent's **display name** in the `/workflows` progress tree. Omit it, and the runtime auto-numbers it (like `agent #3`); pass it, and the tree shows the label you gave.

```javascript
await agent('Review the permission-check logic in auth.ts', { label: 'review:auth' })
```

It's purely **for humans**, affecting no execution behavior. But in a workflow that fans out dozens of agents, a good `label` is the difference between "reading the progress tree" and "staring blankly at a pile of `agent #1…#40`."

Look at how `label` is used in the real-run `parallel-demo` (Run ID `wf_52957913-6d2`, see `assets/transcripts/primitives.md`) — it embeds the dimension name into the label, making the three concurrent agents clear at a glance on the tree:
...
**A practical pattern for label: `type:instance`.** Naming like `review:auth.ts`, `smell:naming`, `verify:race-condition` with "prefix + colon + concrete object" makes the progress tree naturally cluster into visual groups by prefix; a glance tells you "which are review, which are verify, and how far each has gotten." The three real runs in `assets/transcripts/primitives.md` (`smoke` / `smell:*` / `find:* / verify:*`) all use this pattern.

</div>

`label` (the display name) and the previous chapter's `phase` (which group it goes into) are two orthogonal things: `label` decides **what text is written on the leaf**, `phase` decides **which branch the leaf hangs on.** The next section covers `phase`.

---

## 6.4 `schema`: Turning an agent into a "Structured Data Source"

`schema` is `agent()`'s weightiest option, and one of the core capabilities distinguishing Workflow from "manually spinning up subtasks." We've already seen its return effect in 6.2.2; here we clarify its **mechanism** and **when to use it.**
...
2. The runtime **forces** this subagent to deliver its result through an internal `StructuredOutput` tool (rather than writing free text).
...
This means: **you can get type-safe structured data from a language model without writing any parsing code or any error-handling branch.** In the world without a schema, you'd have to make the model "output JSON," then `JSON.parse` yourself, `try/catch` yourself, handle "the model said one extra sentence of nonsense that broke the JSON parse" yourself — schema folds all of this into the runtime.
...
**When should you pass a schema? The criterion is: do you intend to "consume" this product with code?** If the product will have its fields read by later code, drive conditional branches, or be fed into the next agent's prompt — pass a schema, get a clean object. If you just want a piece of natural language for a human (the prose of a final report, an explanation) — don't pass one, take the text. In a practical workflow, the intermediate stages almost always carry a schema (because they're chained programmatically); only the final "for a human" step might return plain text.

</div>

`schema` can also **combine** with `agentType` (6.8) — making a custom-typed subagent return structured data too. That's detailed in 6.8. Schema's full power (`enum`, nested validation, the boundary of the retry mechanism) is the subject of Chapter 07.

---

## 6.5 `phase`: Explicit Grouping in Concurrent Scenarios

We covered the `phase` option in depth in Chapter 05, §5.5.1; here we nail it again from `agent()`'s perspective, because it's the option **most easily missed when writing concurrent workflows, and missing it scrambles the progress tree.**
...
- In **sequential code**, switching the current phase with the global `phase('X')` suffices; subsequent agents group in automatically.
...
This is exactly the form of the real-run `pipeline-demo` (Run ID `wf_bf086b98-6ec`):
...
The strings in `phase: 'Find'` / `'Verify'` must likewise **match `meta.phases[].title` exactly** (case, spaces, letter-for-letter) — the mechanism repeatedly stressed in Chapter 05, §5.5.

<div class="callout warn">

**The relationship between `opts.phase` and the global `phase()` isn't "either-or" but "prefer `opts.phase` in concurrency."** You can perfectly well write a `phase('Find')` before `pipeline` as a fallback, but what actually decides each concurrent agent's grouping is its own `opts.phase`. When both are present, **the `opts.phase` attached to the agent is the reliable one**, because it's unaffected by concurrent interleaving.
...
When an agent's job is simple — classify, extract, format-convert, decide a boolean — using a strong model is wasteful. Drop it to `'haiku'`:
...
This rule is confirmed by three real runs (same session, see `assets/transcripts/primitives.md`):
...
Imagine a workflow: you want 5 agents to **concurrently** modify code each on their own (e.g., fixing 5 different bugs separately, each editing files). If they all write files in the **same working directory**, they trample each other — A edits `utils.js`, B also edits `utils.js`, git state and file content pollute each other, and the result is unpredictable.

`isolation: 'worktree'` gives each such agent its **own git worktree** (same repository, independent working directory) where it edits files without interfering with the others. Per `_grounding.md`:
...
"Expensive" isn't rhetoric. Creating a git worktree involves real disk operations and git overhead. Combined with the magnitude given in this book's writing context: **each worktree starts at about 200–500ms, plus per-agent disk usage.** When you add `isolation: 'worktree'` to all 50 agents, this overhead accumulates into considerable latency and disk consumption.

So the criterion for use is very clear — **use it only when all three conditions "parallel + edits files + would collide" hold at once:**
...
A thoughtful detail: `_grounding.md` states "**auto-cleaned if no changes.**" That is, if an agent with `isolation: 'worktree'` finishes having **produced no file changes**, the runtime automatically cleans up that temporary worktree, leaving no garbage. This lowers the cleanup cost of "added a worktree but found it unnecessary" — but it **cannot** be a reason to "add worktrees casually," because the creation overhead has already been paid.

<div class="callout warn">

**Don't add `isolation: 'worktree'` by default.** The vast majority of agents are **read-only analysis** (review, research, summarize, judge) — they don't write files at all, so there's naturally no collision, and adding a worktree is pure waste. Even when writing files, as long as each agent writes **mutually unrelated independent files**, isolation is usually unneeded. This option is an escape hatch prepared for the **specific** scenario of "parallel modification that would collide," not standard equipment. The full pattern and trade-offs of worktree isolation are the subject of Chapter 19.
...
`agentType` makes this agent not use the default generic subagent but adopt a **named custom type.** Per `_grounding.md`:

> `opts.agentType` uses a custom subagent type (e.g., `'Explore'`, `'code-reviewer'`, combinable with schema).

### 6.8.1 What problem it solves

The Claude Code ecosystem has some **preconfigured subagent types** — they carry a specific system prompt, toolset, or behavioral orientation. For example:
...
When you want an agent to **act in a certain specialized role** rather than start from a generic subagent, specify it with `agentType`:
...
Having covered all the options, let's return to a core question running through the book: **why does fanning out work with `agent()` protect your main loop's context?**
...
- Whereas each subagent dispatched by `agent()` runs in **its own independent context.** It read 100,000 lines of code, generated a big chunk of analysis — those bytes all stay in **its own** context. After it finishes, **only its return value** (that text or validated object) returns to your main loop.
...
This is the essence of `agent()`'s "context protection": **isolate the "process bytes" that would pollute the main loop (the raw material read, the intermediate reasoning) in the subagent's one-off context, and let only the "result bytes" (the distilled return value) flow back.** A question that requires reading 20 files to answer — you needn't read 20 files into the main loop; dispatch an agent to read and think, and it brings back only the answer.

### 6.9.2 The "final text is the return value" design

An ordinary subtask returns a piece of writing **for a human** ("Sure, I've looked it over for you; this file mainly does…"). But a Workflow subagent is explicitly told: **your final output is the program's return value, not pleasantries for a human.** Per `_grounding.md`:
...
This design makes `agent()`'s return value **suitable for consumption by a program** rather than reading by a human — the prerequisite for it to be a "building block of deterministic orchestration."

<div class="callout tip">

**A practical corollary: let agents do the "re-read, re-think" dirty work.** Any task that "requires swallowing a large context to reach a small conclusion" — reading through a big module, scanning a batch of logs, studying a long document — is suited to be tossed to `agent()`. It digests the raw material in an independent context and brings only the conclusion back to the main loop. This is precisely why, during large-scale `parallel` / `pipeline` fan-out, the main loop's context barely grows — and the fundamental advantage of Workflow over "reading hard in the main loop."
...
Real `agent()` calls often use multiple options **at once.** The example below (illustrative, not run) combines this chapter's options as much as possible and marks the intent of each:
...
Reviewing this combination's decisions item by item:
...
This table is itself a demonstration of "choosing agent options": **every option should have a clear "why used / why not," rather than being piled on by feel.** Especially `isolation` — its "not set" is a conscious decision just as much as the others' "set."
