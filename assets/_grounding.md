...
- `opts.isolation:'worktree'`：**昂贵（~200–500ms 启动 + 磁盘/agent）**；无改动自动清理；**工具结果信封层报告 worktree 路径与分支；但脚本里 `agent()` 拿到的是 agent 常规返回值（无 schema 即 string），不是 `{path,branch}` 对象**（实测 `wf_17307da4-707`，详见下方 R9 探针条与 app-a §A.5）。
